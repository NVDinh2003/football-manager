...
@Service
public class FinancialRecordService extends BaseService<FinancialRecord, FinancialRecordDTO, FinancialRecordFilter, UUID> {
    ...
    public FinancialRecordDTO create(FinancialRecordDTO financialRecordDTO) {
        if (memberService.isCurrentUserNotManagerPermissionOfTeam(financialRecordDTO.getTeamId()))
    ...
    public void deleteById(UUID uuid) {
        FinancialRecord financialRecord = financialRepository.findById(uuid)
                .orElseThrow(() -> new EntityNotFoundException(Constants.ENTITY_NOT_FOUND));
        if (memberService.isCurrentUserNotManagerPermissionOfTeam(financialRecord.getTeam().getId()))
            throw new AccessDeniedException(Constants.NOT_MANAGER_PERMISSION);
        super.deleteById(uuid);
    }

    public Page<FinancialRecordDTO> findAllByTeam(FinancialRecordFilter filter) {
        teamRepository.findById(filter.getTeamId())
                .orElseThrow(() -> new EntityNotFoundException(Constants.ENTITY_NOT_FOUND));

        Page<FinancialRecord> list = financialRepository.findAllWithFilter(filter.getPageable(), filter);
        return financialMapper.convertPageToDTO(list);
